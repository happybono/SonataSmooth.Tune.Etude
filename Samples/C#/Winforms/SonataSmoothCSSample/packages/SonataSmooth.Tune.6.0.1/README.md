...
This restriction applies retroactively and prospectively to versions of this library, regardless of the date of disclosure or commit history.
It is equally applicable to all library versions without exception.
...
- Configurable boundary modes : Symmetric (mirror), Replicate (clamp), ZeroPad (explicit zeros), Adaptive (full in‑range sliding window; weighted filters center kernel on position i; SG builds per-position asymmetric coefficients; no synthetic samples).
...
Core conductor exports always include the line "Alpha Blend : {alpha}" even if Binomial Average, Binomial Median, Gaussian Weighted Median, or Gaussian are disabled (Rectangular and Savitzky-Golay ignore alpha).
...
- `Adaptive` : Slides the full window fully inside the valid range; uses only real samples (no synthetic padding). Weighted filters (Binomial, Gaussian) use the original pre-computed kernel centered on position `i`; SG builds per-position asymmetric polynomial coefficients.

> **Historical Note** :   
Earlier UI tooltips referenced "Adaptive (Local Poly + Median)" implying SG-only behavior. Current tooltip reads "Adaptive (real samples only, no boundary synthesis)". Implementation applies Adaptive sliding uniformly to all enabled filters.
...
- **Rectangular** : Sums the in-range samples and divides by count. Uniform weights mean no kernel peak to center; the only phase shift near edges is mathematical (from the asymmetric window extent).
- **Binomial Average / Binomial Median** : The original full-width binomial coefficient array (`binom[]`, length = 2r + 1) is pre-computed once. For each position `i`, weights are extracted via `binom[(pos - left) + r]`, mapping offset 0 (position `i` itself) to `binom[r]` (the kernel peak). This preserves kernel centering on `i` and avoids the additional phase shift that would result from recomputing a shorter kernel.
- **Gaussian / Gaussian Weighted Median** : The Gaussian kernel is computed per position using `offset = pos - left` relative to position `i`, with the original sigma (= windowSize / sigmaFactor). The peak (`offset = 0`) always corresponds to `i`, ensuring no kernel-induced phase shift.
- **Savitzky-Golay** : Per-position asymmetric polynomial coefficients are computed (cached) for (left, right) around position `i` via `ComputeSGCoefficientsAsymmetric`. The polynomial fit is always centered on `i`.
- Benefits : avoids artificial edge energy inflation / attenuation.
- Trade‑off : near edges the window is asymmetric, producing inherent mathematical phase shift from the unequal left/right extents. Weighted filters minimize this by centering the kernel peak on the target position.
...
> **Note** :    
Adaptive boundary handling applies equally to derivative kernels; per-position asymmetric polynomial fits (SG) and centered kernel extraction (Binomial, Gaussian) are used for all supported filters.
...
| Gaussian Weighted Median | Robust median with Gaussian emphasis (kernel centered on position i in Adaptive) | O(w log w) |
...
public enum BoundaryMode
{
    ...
    Adaptive    // Slide window fully inside data (no synthetic samples); weighted filters reuse original kernel centered on position i; SG builds per-position asymmetric coefficients
}
```
...
- Choose `alpha` carefully :
  ...
  - `alpha < 1.0` to retain some original signal and reduce oversmoothing in Binomial Average / Median / Gaussian Weighted Median / Gaussian.
...
- Adaptive boundary mode kernel centering fix :
  - Weighted filters (Binomial Average, Binomial Median, Gaussian, Gaussian Weighted Median) now use the original pre-computed kernel restricted to available offsets and centered on position `i`, instead of recomputing a shorter kernel centered on the truncated window. This eliminates implementation-induced phase shift; only the inherent mathematical asymmetry from the window extent remains.
  - Rectangular (uniform weights) and Savitzky-Golay (asymmetric polynomial coefficients) were already correct and remain unchanged.
- Backward compatibility : no breaking changes; existing smoothing overloads and tuple order unchanged.
- Parameter order unification : `ExportCsvAsyncFromResults` bool filter parameters reordered from `(doMed, doGauss, doGaussMedian, doSG)` to `(doMed, doGaussMedian, doGauss, doSG)`, matching `ExportCsvAsync` and `ExportExcelAsync`. All three export methods now share a consistent `(doRect, doAvg, doMed, doGaussMedian, doGauss, doSG)` signature. Callers using named arguments are unaffected; callers using positional arguments for `ExportCsvAsyncFromResults` must update the order.
